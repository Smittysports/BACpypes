...
from .basetypes import AccessCredentialDisable, AccessCredentialDisableReason, \
    ...
    TimerState, TimerStateChangeValue, TimerTransition, PriorityValue
    ...
    def ReadProperty(self, obj, arrayIndex=None):
        ...
        if arrayIndex is not None:
            ...
            if value is not None:
                try:
                    # There is a bug where the ArrayOf(CharacterString) is not putting the count as [0]
                    # This should be fixed when time allows, until then there is a workaround
                    if ( (self.identifier == 'stateText') or
                         (self.identifier == 'exceptionSchedule') or
                         (self.identifier == 'shedLevelDescriptions')):
                        if (arrayIndex == 0):
                            value = len(value)
                        else:
                            value = value[arrayIndex - 1]
                    else:
...
class LargeAnalogValueObject(Object):
    ...
    properties = \
        ...
        , ReadableProperty('units', EngineeringUnits)
        , OptionalProperty('priorityArray', PriorityArray)
        , OptionalProperty('relinquishDefault', Double)
        , OptionalProperty('covIncrement', Double)
        , OptionalProperty('timeDelay', Unsigned)
        , OptionalProperty('notificationClass', Unsigned)
...
class NotificationClassObject(Object):
    ...
    properties = \
        [ ReadableProperty('notificationClass', Unsigned)
        , ReadableProperty('priority', ArrayOf(Unsigned),3)
