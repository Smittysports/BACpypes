...
#               xpdu.pduSource.pduRoute = pdu.pduSource
    ...
    def process_task(self):
        ...
        if _debug: BIPForeign._debug("process_task: %r", _time())
        if self.bbmdAddress:
            pdu = RegisterForeignDevice(self.bbmdTimeToLive)
            pdu.pduDestination = self.bbmdAddress
            # send it downstream
            ...
            interval = 10    # every 10 seconds or ttl, whichever is smaller
            if interval > self.bbmdTimeToLive:
                interval = self.bbmdTimeToLive
            self.install_task(_time()+interval)
